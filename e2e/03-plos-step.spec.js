...
test.describe("Step 2: PLOs - Standard Mappings", () => {
  ...
  test("should show standard mapping options when standard is selected", async ({ page }) => {
    ...
    await page.getByTestId(/^plo-textarea-/).first().fill("Test PLO");
    await page.waitForTimeout(300);
  ...
  test("keeps multiple PLO panels open after add mapping (re-render)", async ({ page }) => {
    ...
    await page.getByTestId(/^plo-textarea-/).last().fill("PLO 1 text");
    ...
    await page.getByTestId(/^plo-textarea-/).last().fill("PLO 2 text");
    await page.waitForTimeout(120);
    ...
    await page.getByTestId(/^add-mapping-/).first().click();
