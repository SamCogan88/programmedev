...
test.describe("Step 7: MIMLOs (Module Intended Learning Outcomes)", () => {
  ...
  test("should delete a MIMLO", async ({ page }) => {
    ...
    if (await addBtn.isVisible()) {
      await addBtn.click();
      await page.waitForTimeout(200);
      await page.getByTestId(/^mimlo-input-/).first().fill("Test MIMLO");
      await page.waitForTimeout(500);
    }
  ...
  test("should switch between modules", async ({ page }) => {
    ...
    await page.getByTestId(/^module-code-/).nth(1).fill("CMP8002");
    await page.getByTestId(/^module-title-/).nth(1).fill("Databases");
    await page.waitForTimeout(600);
...
test.describe("Step 7: MIMLO Learning Outcome Linting", () => {
  ...
  test("should highlight weak verbs in MIMLO text", async ({ page }) => {
    ...
    if (await addBtn.isVisible()) {
      ...
      await page.getByTestId(/^mimlo-input-/).first().fill("Understand the basics of programming");
      await page.waitForTimeout(600);
