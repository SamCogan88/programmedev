...
test.describe("Step 9: Assessments", () => {
  test.beforeEach(async ({ page }) => {
    ...
    if (await addMimloBtn.isVisible()) {
      ...
      await page.waitForTimeout(200);
      await page.getByTestId(/^mimlo-input-/).first().fill("Design and implement software solutions");
      await page.waitForTimeout(400);
    }
  ...
  test("should add an assessment", async ({ page }) => {
    await page.getByTestId(/^add-asm-/).first().click();
  ...
  test("should validate weightings sum to 100%", async ({ page }) => {
    ...
    // Set first weight
    await page.getByTestId(/^asm-weight-/).first().fill("60");
    await page.waitForTimeout(300);
