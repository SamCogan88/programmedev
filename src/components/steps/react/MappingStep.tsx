...
const MimloCheckbox: React.FC<MimloCheckboxProps> = ({
  ...
  return (
    <label
      className={`list-group-item d-flex gap-2 align-items-start ps-4${isDisabled ? " opacity-50" : ""}`}
    >
      <Form.Check
        type="checkbox"
        className="m-0 mt-1"
...
export const MappingStep: React.FC = () => {
  ...
  return (
    <Card className="shadow-sm">
      <Card.Body>
        <h5 className="card-title mb-3" id="mapping-heading">
          <Icon name="graph" className="me-2" aria-hidden />
          Map PLOs to MIMLOs (QQI-critical)
        </h5>
        ...
        <div
          ...
          data-testid="mapping-accordion"
        >
          <Accordion
            id="mappingAccordion"
            defaultExpandedKeys={plos.length > 0 ? [plos[0].id] : []}
          >
