  ...
  return (
    <div className="alert alert-secondary mb-3 small">
      ...
      <div className="d-flex flex-wrap gap-1">
        {verbs.map((v) => (
          <span key={v} className="badge text-bg-light">
            {v}
          </span>
        ))}
      </div>
  ...
  return (
    <div className="plo-lint-warnings mt-2" role="status" aria-live="polite">
      {warnings.map((issue, idx) => (
        <div key={idx} className="alert alert-warning py-1 px-2 mb-1 small">
      ...
      <div className="mt-2" role="list" aria-label={`Current mappings for PLO ${ploIndex + 1}`}>
        {currentMappings.length === 0 ? (
              ...
              <div key={stdId} className="mb-2">
                ...
                <div>
                  {items.map(({ mapping, index }) => (
                    <span key={index} className="badge text-bg-light border me-2 mb-2">
          ...
          currentMappings.map((m, i) => (
            <span key={i} className="badge text-bg-light border me-2 mb-2">
              {m.criteria} / {m.thread}
              <button
                type="button"
                className="btn btn-sm btn-link text-danger p-0 ms-2"
          ...
          {plos.map((plo, i) => {
            const maps = (plo.standardMappings ?? []).map((m) => {
              ...
              const stdName = hasMultipleStandards ? (
                <span className="badge text-bg-info me-1">{standardNames[stdIdx] ?? stdId}</span>
              ) : null;
              return (
    ...
    <Card className="shadow-sm">
      <Card.Body>
        <div className="d-flex justify-content-between align-items-center mb-3">
          <h5 className="card-title mb-0" id="plos-heading">
            <Icon name="list-checks" className="me-2" aria-hidden />
            Programme Learning Outcomes (PLOs) (QQI-critical)
          </h5>
          <Button
            variant="dark"
            size="sm"
