    ...
    <Card className="shadow-sm">
      <Card.Body>
        <h5 className="card-title mb-3">
          <Icon name="books" className="me-2" aria-hidden />
          Reading Lists
        </h5>
        ...
        {modulesForEdit.length === 0 ? (
          ...
          <>
            <AccordionControls
              accordionId="readingAccordion"
              onExpandAll={() => setExpandedModules(new Set(modulesForEdit.map((m) => m.id)))}
              onCollapseAll={() => setExpandedModules(new Set())}
            />
            <Accordion
              id="readingAccordion"
