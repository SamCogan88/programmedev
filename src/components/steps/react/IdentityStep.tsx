...
import { Form, Card, Row, Col, Button, InputGroup, Badge } from "react-bootstrap";
import {
  Accordion,
  AccordionControls,
  AccordionItem,
...
const ElectiveGroupRow: React.FC<ElectiveGroupRowProps> = ({
  ...
  return (
    <Row
      ...
      data-testid={`elective-group-row-${group.id}`}
    >
      <Col xs="auto">
...
export const IdentityStep: React.FC = () => {
  ...
  const [expandedDefinitions, setExpandedDefinitions] = useState<Set<string>>(
    new Set([programme.electiveDefinitions?.[0]?.id ?? ""].filter(Boolean)),
  );
  ...
  const handleAddDefinition = useCallback(() => {
    ...
    saveDebounced();
    // Expand the new definition
    setExpandedDefinitions((prev) => new Set([...prev, newDef.id]));
  ...
  const toggleDefinition = useCallback((defId: string) => {
  ...
  return (
    <>
      ...
      <Card className="shadow-sm">
        <Card.Body>
          <h5 className="card-title mb-3" id="identity-heading">
            <Icon name="identification-card" className="me-2" aria-hidden />
            Identity (QQI-critical)
          </h5>
      ...
      <Card className="shadow-sm mt-3">
        <Card.Body>
          <div className="d-flex justify-content-between align-items-center mb-3">
            <h5 className="card-title mb-0" id="elective-defs-heading">
              <Icon name="path" className="me-2" aria-hidden />
            ...
            {definitions.length === 0 ? (
              ...
              <>
                <AccordionControls
                  ...
                  onExpandAll={() => setExpandedDefinitions(new Set(definitions.map((d) => d.id)))}
                  onCollapseAll={() => setExpandedDefinitions(new Set())}
                />
                <Accordion id="electiveDefinitionsAccordion">
                  {definitions.map((def, defIdx) => (
                    <ElectiveDefinitionItem
                      key={def.id}
                      definition={def}
                      definitionIndex={defIdx}
                      isExpanded={expandedDefinitions.has(def.id)}
